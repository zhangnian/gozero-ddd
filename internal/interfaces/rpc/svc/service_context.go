...
func NewServiceContext(c config.RpcConfig) *ServiceContext {
	log.Println("🚀 [gRPC ServiceContext] 开始初始化服务上下文...")

	// 1. 创建基础设施层容器
	// 负责：数据库连接、仓储实现、事件总线、领域服务
	infra := infracontainer.NewInfrastructureContainer(&rpcConfigAdapter{c})
	...
	log.Println("✅ [gRPC ServiceContext] 服务上下文初始化完成")
...
// rpcConfigAdapter RPC 配置适配器
// 将 config.RpcConfig 适配为 InfraConfig 接口
type rpcConfigAdapter struct {
	config.RpcConfig
}
...
func (a *rpcConfigAdapter) IsAutoMigrate() bool {
